...
int main(void)
{
    ...
    while (1) 
    {
		...
		if (B_elapsedTime >= 1000) { //3000 ms
			threeLED_Tick();
			B_elapsedTime = 0;
		}
		if (P_elapsedTime >= 400) {//1000 ms
			blinkLED_Tick();
			P_elapsedTime = 0;
		}
		if (S_elapsedTime >= 400) {//1000 ms
			blinkLED_Tick();
			P_elapsedTime = 0;
		}
